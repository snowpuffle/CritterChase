...
class BFSPathFinderTest {
    ...
    void doesNotGoOutsideBoard() {
        ...
        assertEquals(5, distances[0].length);

        // Verify Top-Left Position Is the Starting Position
        assertEquals(0, distances[0][0]);

        // Verify Bottom-Right Position Is Reachable
        assertEquals(8, distances[4][4]);

        // Verify No Extra Rows Exist Outside the Board
        assertEquals(5, distances.length);

        // Verify No Extra Columns Exist Outside the Board
        assertEquals(5, distances[0].length);
    }
    ...
    void findsPathAroundWall() {
        ...
        // Create Wall Directly to the Right of Player
        board.setGameObjectAt(
                new Wall(0, 1, "wall.png"));
        ...
        // Verify BFS Finds an Alternate Path Around the Wall
        assertEquals(4, distances[0][2]);

        // Verify Position Below the Player Is Reachable
